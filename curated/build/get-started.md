---
title: Get started with meta-pantavisor
sidebar_position: 1
description: Clone meta-pantavisor and build a Pantavisor image with KAS — repository setup, git worktrees, build modes, common targets, and a smoke test.
---
...
```bash
# Interactive configuration menu — writes your choices to .config.yaml
./kas-container menu Kconfig

# Build from the generated .config.yaml
...
If you have `kas` installed on the host you can use `kas menu Kconfig` /
`kas build .config.yaml` directly instead of the `kas-container` wrapper.

### Workspace Build (local source development)

Adds `kas/with-workspace.yaml` to create a devtool workspace with editable pantavisor source. See [pantavisor-development.md](../develop/pantavisor-development.md).
...
| `pantavisor-appengine-distro` | Full test distribution tarball (appengine + tester + netsim images + `test.docker.sh`) — see [automated-workflow.md](/reference/meta-pantavisor/testing/automated-workflow) |
...
If you include meta-pantavisor in your own Yocto project without KAS, source
your project's `oe-init-build-env` and build the Pantavisor targets as normal
recipes. (The `layers/poky/` path below is where KAS checks out poky inside a
meta-pantavisor build tree — in your own project, substitute the path to your
poky/openembedded-core checkout.)
...
After building the appengine image, see [../testing/development-workflow.md](/reference/meta-pantavisor/testing/development-workflow) for the manual dev workflow, or [../testing/automated-workflow.md](/reference/meta-pantavisor/testing/automated-workflow) for structured automated testing.
