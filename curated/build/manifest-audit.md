...
sidebar_position: 4
description: pv-manifest-audit.bbclass produces a deterministic rootfs manifest and fails the build on unexpected drift — setuid binaries, dropped files, ownership flips.
---
...
2. The build prints a full-add patch to the bitbake log and writes
   `${DEPLOY_DIR_IMAGE}/${IMAGE_NAME}.manifest.patch` (i.e.
   `deploy/images/<MACHINE>/`). It writes there directly — not via
   `IMGDEPLOYDIR` — so the patch survives the strict-mode build failure.
