...
This guide will walk you through setting up **Pantavisor** on a **Raspberry Pi 3B**. While Pantavisor supports a variety of embedded devices, the steps here are tailored for the Raspberry Pi 3B. The same principles apply to other boards — see [Install on hardware](/install) for board-specific guides.
...
> **Compatibility Check**: To avoid issues, always verify that your device is officially supported by Pantavisor before you download and flash the image. You can find a complete list of supported platforms on the [supported hardware page](/install/supported-devices).
...
2.  **Connect** a USB-to-TTL serial adapter to the Pi's console pins (GPIO14 TX → adapter RX, GPIO15 RX → adapter TX, GND → GND, 115200 baud) — see [Serial port access](/operate/device-access/serial-port).
...
5.  **Next:** go to [Device access](/operate/device-access) for first-boot instructions.

## Next Steps

- **Claim your device on Pantahub** for remote management — see [Remote access via Pantahub](/operate/device-access/remote-pantahub).
- **Install your first application** with `pvr` — see [Install applications](/develop/application/install/).